
// const requestOptions = {
//   method: 'GET',
//   redirect: 'follow',
//   headers: {
//       //  "content-[type]": "application/json",
//       "Content-Type": "application/json",
//       //  Accept:'application/json',
//         Authorization: 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..c4VGCiyV6bOTwJ8FFe9JW-4BnG3EkD9nKVMwSrLBxis',
//         'Allow CORS': 'Access-Control-Allow-Origin '*''
//       } 
      
// };

// fetch("http://hemis.ethernet.edu.et/backend/StudentGraduate/GetStudentGraduatesByInstitution/202122/AASTU", requestOptions)
//   .then(response => response.json())
//   .then(result => console.log(result))
//    .catch(error => console.log('MY API error is', error));
  
 // new 

function getUser() {
  return fetch('http://hemis.ethernet.edu.et/backend/StudentGraduate/GetStudentGraduatesByInstitution/202122/AASTU', {
    method: 'GET',
    headers: {
      Accept: 'application/json',
      Authorization: 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..c4VGCiyV6bOTwJ8FFe9JW-4BnG3EkD9nKVMwSrLBxis',
      'Content-Type': 'application/json',
      'Allow CORS': 'Access-Control-Allow-Origin '*''
    },
    
  })
  
  .then(response => response.json())
    .catch(error => {      console.log('MY API error: ', error);
    });
}


getUser().then(data => {
  console.log(data);
  
});

